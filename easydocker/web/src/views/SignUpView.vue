...
const checkDuplicate = async () => {
    if (isEmailValid.value) {
        try {
            const endpoint = `/api/v1/checkDuplicate?userEmail=${email.value}`;
...
const signup = async () => {
    ...
    try {
        const response = await api.post('/api/v1/signup', requestData.value);
...
const login = async () => {
    ...
    try {
        const response = await api.post('api/v1/authentication', requestData2.value);
